<template>
  <div>
    ...
    <div class="play-title-box">
      ...
      <span class="play-title-span"><strong><p id="title">{{ album.name }}</p></strong></span>
    ...
    <div id="background-img-and-title-box">
      <div id="img-box">
        <img :src="album.profilePicture" id="background-img" />
      </div>
    </div>
    <div id="song-list-content">
      <div id="title-box">
        <span id="title-span"><strong>{{ album.name }}</strong></span>
        <span id="introduction-span">{{ album.detail }}</span>
      </div>
      <div id="owner-box">
        <div id="owner-profile-picture-box">
          <img :src="album.singerList[0].profilePicture" width="30"/>
        </div>
        <span id="owner-span">{{ album.singerList[0].name }}</span>
        <div id="follow-box">
      ...
      <div id="song-list-items">
          <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
            <song v-for="(song, index) in songListList" :key="song" :show-step="false" :rank-number="index + 1" :name="song.name" :author="song.author" :album="song.album" :id="album.id"></song>
          </van-list>
      </div>
    </div>
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      if (route.value.params.id) {
        albumApi.getAlbumAndSongList(route.value.params.id).then(
          response => {
            songListList.value = response.data.mediaList
